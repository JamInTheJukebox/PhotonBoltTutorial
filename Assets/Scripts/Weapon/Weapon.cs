...
public class Weapon : MonoBehaviour
{
    ...
    protected PlayerMotor _playerMotor;     // let the other player know they took damage.
    protected PlayerCallback _playerCallback; // call fire effect

    protected int _fireFrame = 0;
    private Coroutine _reloadCrt = null;
    protected Dictionary<PlayerMotor, int> _dmgCounter; // stack damage.
        ...
        if (_currentAmmo >= _weaponStat.ammoPerShot)
        {
            if (_fireFrame + _fireInterval <= BoltNetwork.ServerFrame)
            {
                ...
                if (_playerCallback.entity.IsOwner)
                    _playerCallback.FireEffect(WeaponStat.precision, seed);     // if we are the owner, simulate it on other players screen, otherwise, simulate it locally on the controller's screen.
                if (_playerCallback.entity.HasControl)
                    FireEffect(seed, WeaponStat.precision);     // play animation on controller's view so we send less events to clients(clients-1).
                ...
                for (int i = 0; i < _weaponStat.multiShot; i++)
                {
                    ...
                    if (Physics.Raycast(r, out rh, _weaponStat.maxRange))
                    {
                        ...
                        if (target != null)     // IF THE TARGET THAT THE RAHYCAST HIT IS NOT A PLAYER, DO NOTHING! 
                        {
